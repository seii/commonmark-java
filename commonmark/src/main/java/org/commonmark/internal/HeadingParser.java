...
public class HeadingParser extends AbstractBlockParser {
        ...
        public BlockStart tryStart(ParserState state, MatchedBlockParser matchedBlockParser) {
            ...
            if (line.getContent().charAt(nextNonSpace) == '#') {
                ...
                if(!Parsing.IS_ROUNDTRIP) {
                    ...
                    // AST: Parse full line, not a subset
    ...
    private static HeadingParser getAtxHeading(SourceLine line) {
        ...
        if(Parsing.IS_ROUNDTRIP) {
            // AST: Capture whitespace before heading delimiter(s)
        ...
        if(Parsing.IS_ROUNDTRIP) {
            // AST: Capture whitespace between heading delimiter(s) and
            //      actual text contents. This will be concatenated into
            //      the full content at the end of this method.
        ...
        while (scanner.hasNext()) {
            ...
            switch (c) {
                case '#':
                    if (hashCanEnd) {
                        ...
                        if (scanner.hasNext()) {
                            ...
                            if(Parsing.IS_ROUNDTRIP) {
                                // AST: This could be an empty line with whitespace
                                //      of different kinds, so just find the end
                                scanner.find(Scanner.END);
                            }
                            end = scanner.position();
                            
                            if(Parsing.IS_ROUNDTRIP) {
                                // AST: Since this isn't a closing sequence, any
                                //      whitespace will be considered part of the
                                //      content and will be passed literally for
                                //      inline parsing
                                numEndingSymbol = 0;
                ...
                case '\n':
                    if(Parsing.IS_ROUNDTRIP) {
                        // AST: Whitespace is significant, make sure it counts
                        hashCanEnd = true;
                        whitespacePostContent = scanner.whitespaceAsString();
                        end = scanner.position();
                        break;

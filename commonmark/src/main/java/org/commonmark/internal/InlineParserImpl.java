...
public class InlineParserImpl implements InlineParser, InlineParserState {
        ...
        if (inlineParsers != null) {
            ...
            for (InlineContentParser inlineParser : inlineParsers) {
                ...
                if(Parsing.IS_ROUNDTRIP && inlineParser instanceof EntityInlineParser) {
                    // AST: When parsing to HTML it is correct to parse entities,
                    //      but the CommonMark spec shows the AST as rendering
                    //      literals, not the parsed entities
    ...
    private String parseLinkDestination(Scanner scanner) {
        ...
        if(!Parsing.IS_ROUNDTRIP) {
            ...
            // spec: URL-escaping should be left alone inside the destination, as
            //       all URL-escaped characters are also valid URL characters
            // AST: No need to chop off text, it can all be parsed
    ...
    private String parseLinkTitle(Scanner scanner) {
        ...
        if(!Parsing.IS_ROUNDTRIP) {
            ...
            // AST: Parse full content, not a substring
            return rawTitle;
        }
    }
    ...
    private Node parseText() {
        ...
        if(!Parsing.IS_ROUNDTRIP) {
          if (c == '\n') {
            ...
            // AST: Keep the tracking of trailing spaces in case it's needed
            //      for a hard line break, but parse full content
            if (c == '\n') {
                // We parsed until the end of the line. Trim any trailing spaces and remember them (for hard line breaks).
                int end = Parsing.skipBackwards(' ', content, content.length() - 1, 0) + 1;
                trailingSpaces = content.length() - end;
